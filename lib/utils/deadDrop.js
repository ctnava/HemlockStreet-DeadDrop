...
async function uploadEncrypted(fileName, secret) {
    ...
    // try {
    //     const testPath = './uploads/encrypted/active';
    //     if (fs.readdirSync(testPath).length > 0) {
    //         console.log("resetting testPath");
    //         fs.unlinkSync(testPath, {recursive: true});
    //         fs.mkdirSync(testPath);
    //     }
    //     console.log("testing zipped file...");
    //     unlockZip(pathToArchive, secret, testPath);
    //     console.log("zipped file okay!");
    // } catch (err) { 
    //     console.log(err);
    //     console.log("zipped file corrupted! retrying...");
    //     await uploadEncrypted(fileName, secret); 
    // }
...
async function getFile(cipher, fileName) {
    ...
    if (success === true) {
        ...
        fs.renameSync(`${exportDir}/${files[0]}`, `${exportDir}/${fileName}`);
    }
