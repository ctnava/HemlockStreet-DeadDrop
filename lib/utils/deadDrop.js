...
const { ipfs, CID } = require("../setup/ipfs.js");
...
async function uploadEncrypted(fileName, secret) {
    ...
    console.log("Archived! Opening...");
    // breaks here || File not finalized before opening?
    const file = await promises.readFile(pathToArchive); 
    fs.appendFileSync("zip.zip", file);
    console.log("file", file);
    console.log("Opened! Adding to IPFS...");
    // proof || Saved file is corrupted
    const content = await ipfs.add({content: file});
    console.log("Added! Pinning...");
