...
const altKey = process.env.BC_KEY;
...
function getProvider(chainId) {
  ...
  if (isDev) return new ethers.providers.JsonRpcProvider();
    ...
    switch (chainId) {
      ...
      default:
        endpoint = "unsupported/chainId";
    }
    const rpcUrl = mUrl + endpoint;
    // console.log("rpcUrl", rpcUrl); // COMMENT ME BEFORE PROD
    if (endpoint === "unsupported/chainId") return endpoint;
...
function getContract(address, abi, chainId) {
  ...
  if (provider === "unsupported/chainId") return provider;
